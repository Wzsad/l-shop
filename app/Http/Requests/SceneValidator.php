...
trait SceneValidator
{
    ...
    protected function handleValidate()
    {
        ...
        $instace = $this->getValidatorInstace();
        if ($instace->fails()) {
            $this->failedValidation($instace);
        }
    }
    ...
    {
        $rules = [];
        foreach ($sceneRule as $key => $scene) {
            if (is_numeric($key) && array_key_exists($scene, $rule)) {
                $rules[$scene] = $rule[$scene];
            } else {
                $rules[$key] = $scene;
            }
        }
